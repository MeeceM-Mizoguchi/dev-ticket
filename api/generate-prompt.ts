...
## 実装後のイメージ・動作例
チケット詳細・要件・コメントに記載された情報をもとに、実装後にユーザーが目にする具体的な動作をシナリオ形式で記述する。
チケット情報に記載のある範囲でのみ記述し、記載のない部分は推測で補わないこと。
以下のような形式で、該当するものを記述する（該当しないものは省略する）：

- **ボタン・UI追加**: 「○○画面に「△△」ボタンが追加され、クリックすると〜になる」
- **メッセージ・通知**: 「○○の操作をすると「△△△△」というトースト/モーダル/エラーメッセージが表示される」
- **画面遷移・状態変化**: 「○○を実行すると、画面が〜の状態に切り替わる」
- **データ表示**: 「○○の一覧に△△の項目が表示されるようになる」
