...
//
// ★ ルーティングの経緯（重要）:
//   Office が開く実 URL は /api/dav/<token>/<ファイル名>.xlsx の「2セグメント」。
//   ところが本番 Vercel では api/dav/[...path].ts のキャッチオールが
//   「1セグメントしか」関数に振らず、2セグメント以上は関数に届く前に
//   Vercel 自身の NOT_FOUND になってしまう（＝Office からは「接続できません」）。
...
function pathSegments(req: any): string[] {
  ...
  if (Array.isArray(raw)) return raw.flatMap((s: string) => String(s).split("/")).filter(Boolean);
...
export default async function handler(req: any, res: any) {
  ...
  const segs: string[] = pathSegments(req);
