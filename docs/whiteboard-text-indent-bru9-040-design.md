...
| `Backspace` / `Delete` | **インデントを消しに行った時だけ握り潰す**（文字消しでインデントが崩れない・§3.6） |
...
- **非活性のときは「左揃えにする」ボタンを同時に出す**（`setTextAlignLeft`）。
  標準の「テキストの配置」はプロパティパネルをかなり下までスクロールしないと現れないため、
  非活性のまま放置すると「ボタンは見えるのに押せず、直し方は画面外」という導線になってしまう。
  揃えを**自動では変えない**（§3.4）代わりに、ユーザーが押した時だけ1クリックで変えられるようにする。
...
### 3.5 テキストボックスと図形ラベルの違い

**判定ルール（左揃え・右揃えのみ対応）は両者まったく同じ**で、`textAlign` だけで決まる。
ただし **既定の揃えと、幅の伸び方が違う**ため、体感は次のように分かれる。

| | **テキストボックス**（`isPlainTextBox`） | **図形のラベル**（バインドテキスト） |
|---|---|---|
| 既定の `textAlign` | **`left`**（`DEFAULT_TEXT_ALIGN`・`chunk-4FTI6OG3.js:232`） | **`center`**（`bindTextToContainer` が固定・`index.js:21812`） |
| 作った直後にインデントできるか | **できる**（既定が左揃えなので何もしなくてよい） | **できない**（左か右へ変えてから。パネルは非活性＋ツールチップ） |
| 幅 | `autoResize` で**文字に合わせて伸縮**（`x` は不変） | **図形の幅は固定**（テキストはその中で折り返す） |
| 左インデントの見え方 | 枠が右へ伸び、**文字が右へずれる**（＝添付画像） | 図形はそのまま、**文字が右へずれる** |
| 右インデントの見え方 | **文字は動かず、枠だけ右へ伸びる**（§3.5-注） | **文字が左へ寄る**（右端から離れる） |
| 表のセル | — | 図形ラベルと同じ（幅固定） |

> **注（テキストボックス＋右揃えのときだけの挙動）**:
> `redrawTextBoundingBox` は**素のテキスト要素の `x` を揃えに関わらず変えない**（`chunk-4FTI6OG3.js:14560-14620`）。
> 幅は行末スペースぶん右へ伸びるが、行も同じだけ長くなるので**グリフの画面位置は1ミリも動かない**。
> 結果として「枠の右端と文字の間に余白ができる」＝**枠線・背景（`wbTextBox`）を付けている時にだけ見た目が変わる**。
> 枠線も背景も無い素のテキストボックスを右揃えで右インデントしても、**見た目上は何も起きない**。
>
> これは Excalidraw が右揃えテキストでも `x` 固定で右へ伸びる（＝文字を打ち足したときと同じ）ネイティブ挙動に従った結果。
> ここで `x` を自前で補正すると、パネル操作（自前の `updateScene`）と `Tab` キー（Excalidraw の確定経路）で
> 挙動が食い違うため、**補正しない**。文字を動かしたい場合は左揃えを使うのが素直。

> 図形のラベルは既定が `center` なので、初期状態ではインデントできない。
> パネルに出る **「左揃えにする」ボタン**（§3.2）が1クリックの導線になる。標準の整列ボタンでも同じ。
> **テキストボックスは既定が `left` なので、この一手間は不要**（＝今回の主用途は最初から素直に動く）。

### 3.6 Backspace / Delete をインデントの流儀で処理する（BRU9-040 追補）

**症状**: インデントを入れた行で文字を消していくと、そのまま行頭の空白まで1つずつ削れて揃えが崩れる。

インデントの実体は普通の半角スペースなので、放っておくと**普通の文字として消せてしまう**。
これは「インデントは書式であって文字ではない」というユーザーの認識と食い違う。
`handleIndentDelete` で **2段構え**にする。

#### ① インデントだけで中身が無い行 → 改行ごと畳む

空白を1つずつ削るのではなく、**行をひとまとまりとして扱い、改行ごと消して隣の行へ移る**。

```
  あああ          ←  Backspace 一発で
  |               ←  この行が改行ごと消え、キャレットは上の行末へ
     ↓
  あああ|
```
...
| キー | 動作 |
|---|---|
| `Backspace` | **直前の改行＋この行**を消す → キャレットは**上の行末**へ |
| `Delete` | **この行＋直後の改行**を消す → 下の行が上がってくる |

- キャレットがインデントの**途中**にあっても、1回で行ごと畳む（空白は目に見えないので「1文字ずつ削る」に意味が無い）。
- 上（下）に行が無いときは素通し＝ブラウザ既定に任せる（繋ぐ先が無いのに握り潰すと行き止まりになる）。

#### ② 中身がある行 → インデントに手を掛けた時だけ握り潰す

保護範囲は**その行のインデント側の連続スペース**（左揃え＝行頭、右揃え＝行末）。

| 状況 | 動作 |
|---|---|
| 文字の上で `Backspace` / `Delete` | **通す**（普通に消せる） |
| インデントの内側／境界 | **握り潰す**（インデントは減らない） |
| 行頭で `Backspace`（前の行と結合） | **通す**（行の連結まで禁じない。右揃えは行末 `Delete` が同様） |
| 範囲を**選択**してから削除 | **通す**（ユーザーの明示操作を妨げない） |
| インデントが無い行 | **通す**（従来どおり） |
| 中央揃え（インデント非対応） | **通す**（守る対象が無い） |

インデントを**減らしたいとき**は `Shift+Tab`（または書式パネルの ⇤）を使う。
＝「増やす／減らす」はインデント専用の操作に一本化し、文字消しとは経路を分ける。

> 対象は**編集中の textarea 内のキー操作のみ**。選択削除・切り取り・貼り付けなど、
> ユーザーが明示的にテキストを置き換える操作は従来どおり通る。
> 行を畳む削除も `execCommand` 経由なので、`Ctrl+Z` 一回で元に戻る。
...
//   §3.6。true＝こちらで処理した（握り潰す or 行を畳んだ）。backward: true=Backspace

// ── 要素レベル（非編集中） ──
export function indentTargetOf(el: any, byId: Map<string, any>): any | undefined;  // 図形→そのラベル
...
export function setTextAlignLeft(api: any, ids: string[]): void;                   // 「左揃えにする」
```
...
useEffect(() => {
  ...
  const onKey = (e: KeyboardEvent) => {
    ...
    if (!isTab && !isPlainEnter && !isDelete) return;
    ...
    if (isDelete) {  // §3.6 インデント行を畳む／インデントを守る
      if (!side || !handleIndentDelete(ta, side, e.key === "Backspace")) return;
...
6. **枠線も背景も無いテキストボックスを右揃えにして右インデントしても見た目が変わらない**（§3.5-注）。
   図形ラベル・表セル・枠線付きテキストボックスでは正しく効く。
...
3-2. インデントした行の文字を `Backspace` で全部消す → **インデントは残る**（§3.6-②）。`Shift+Tab` / ⇤ でだけ減らせる。
3-3. 3-2 の状態（インデントだけが残った行）でさらに `Backspace` → **空白が1つずつ削れるのではなく、
     改行ごと畳まれてキャレットが上の行末へ移る**（§3.6-①）。`Ctrl+Z` 一回で戻せる。
4. 図形（四角・楕円・ひし形）にラベルを入れ、**左揃えに変えてから** パネルの `⇥` → **添付画像と同じ見た目**になる。
5. 表のセルでも 1〜3 が同じように動く。

### 右揃え

6. 右揃えの**図形ラベル／表セル**で `⇥` / `Tab` → **右端から文字が離れる**（文字が左へ寄る）。`Shift+Tab` で戻る。
7. 右揃えでインデントした行末で `Enter` → 次の行も**同じだけ右端から離れた位置**から始まる。
8. 右揃えの図形ラベルで、行が図形幅いっぱいになるまで文字を足す → **はみ出さず**、余った行末余白だけが詰まる（§6）。
9. 右揃えの**枠線付きテキストボックス**で `⇥` → **枠の右辺と文字の間に余白ができる**（文字自体は動かない・§3.5-注）。

### 中央揃え（非対応の確認）

10. 中央揃えのテキスト／ラベルでは パネルの ⇤ / ⇥ が**非活性**で、理由がツールチップに出る。
11. 中央揃えで `Tab` を押しても **何も起きない**（Excalidraw 標準の4スペースも入らない）。`Enter` は普通に改行できる。
12. 中央揃えのとき出る **「左揃えにする」を押す → その場で左揃えになり、⇤ / ⇥ が活性化**する。
    図形ラベルの位置も左寄せへ正しく置き直される（1 undo で戻せる）。
13. 標準の「テキストの配置」で中央揃え → 左揃えへ切り替えた瞬間も、パネルのボタンが**その場で活性化**する。

### 共通

14. **テキストボックスは新規作成直後（既定＝左揃え）に、揃えを触らずそのままインデントできる**（§3.5）。
15. **日本語入力**: 変換中の `Enter`（確定）でインデントが入らない／編集が終わらない。`Tab` で変換候補選択を邪魔しない。
16. インデント後に **PNG / SVG / 画像コピー**で書き出す → 余白がそのまま出力される（左右とも）。
17. インデント直後に `Ctrl+Z` → 編集中はその一手だけ戻る／非編集中はインデント前の状態へ1回で戻る。
18. 2ブラウザで同じボードを開き、片方でインデント → **もう片方に正しく反映**され、ちらつかない。
19. インデントで行が伸びて折り返しが増えたとき、図形・表セルの高さが伸び、**減らすと元に戻る**（BRU6-011 の退行が無い）。
20. 背景・枠線付きテキストボックス（`wbTextBox`）でインデントすると、**影矩形（BRU5-062）も追従して伸縮**する。
21. 閲覧専用ユーザーでは `Tab` / パネルのインデント操作が効かない。
22. フレーム名の編集中、カーソルチャット入力中に `Tab` / `Enter` が従来どおり動く（横取りしていない）。
