...
既定は「そのまま保存」とし、確認ダイアログで失われるものを明示する。
...
- **そのまま保存（既定）** … 従来どおり Supabase Storage へ。書式もマクロも保つ
- **Google形式に変換して保存** … Drive へ変換して取り込む。ファイルは1つだけ
...
ファイル本体は**ブラウザから Drive へ直接送る**（再開可能アップロード）。
サーバーは送り先URLを発行するだけで、ファイルを中継しない。
project-files の「署名付きアップロードURL → register」と同じ組み立てで、
Vercel のリクエストサイズ上限(4.5MB)に縛られない。
...
| `upload-session` | POST | Office文書をGoogle形式へ変換して取り込むための、再開可能アップロードの送り先を発行 |
| `register-upload` | POST | 上のアップロード完了後に、権限配布とDB登録を行う |
