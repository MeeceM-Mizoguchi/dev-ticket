...
| ⑪ | Office文書の取り込み | **アップロード時に「そのまま / Google形式に変換」を選ばせる**。既定は「そのまま」。変換すると元ファイルは残らないため、ファイルは常に1つ（⑪補足） |
| ⑫ | Googleファイルの書き出し | **閲覧者自身のGoogleログインで直接ダウンロード**。サーバーを中継しない |
...
### 2.1 「変換コピー」ではなく「変換して取り込む」にした理由

当初は「既存の .xlsx を変換したコピーを作る」案を検討したが、**元ファイルと変換後のファイルが
DevTicket 上に2つ並び、同期しないまま別々に育つ**（どちらが最新か分からなくなる）。

アップロード時に選ばせれば、`project_files` にできる行は常に1つで済む。
代わりに「変換を選ぶと元の .xlsx が DevTicket に残らない」ため、
確認ダイアログで失われるものを明示する。
...
### 5.1.1 Office文書のアップロード時の確認

Office文書（.xlsx/.xls/.csv/.docx/.doc/.pptx/.ppt）を入れたとき、保存形式を選ばせる。

- **そのまま保存** … 従来どおり Supabase Storage へ。書式もマクロも保つ
- **Google〇〇に変換して保存** … Drive へ変換して取り込む。ファイルは1つだけ

ラジオボタンで選ばせ、**初期状態はどちらも未選択**（2.1 のとおり推奨を置かない）。
選択肢の名称は入れたファイルの種別に合わせる（Excel→スプレッドシート / Word→ドキュメント /
PowerPoint→スライド）。複数種が混ざっているときだけ「Google形式」とまとめる。

確認を出すのは「組織が連携を有効にしている」かつ「フォルダごとのアップロードではない」ときだけ。
フォルダごとのアップロードは件数が多く、1件ずつ判断させる意味が薄いので常に「そのまま」とする。

**`.xlsm` は変換の対象にしない。** マクロは変換で必ず失われるうえ、
この方式では元ファイルが DevTicket に残らないため、取り返しがつかなくなる。

### 5.1.2 変換アップロードの経路

```
① ブラウザ → Supabase Storage     既存の署名付きURL（stageProjectFile）
② サーバー  → Storage から取り出す
③ サーバー  → Google Drive         再開可能アップロード（サーバー間）
④ 成功したら Storage 側の実体を削除し、project_files に Google ファイルとして1行登録
```

**ブラウザから Drive へ直接送る形は採らない（本番で実測して失敗した）。**
サーバーで作った再開可能アップロードのセッションURIへブラウザから PUT すると、
応答に `Access-Control-Allow-Origin` が付かず CORS で必ず弾かれる。
一方でファイル本体をサーバーレス関数の body に通すと、Vercel のリクエストサイズ上限(4.5MB)に引っかかる。
そのため、一度 Storage に置いてからサーバー間で中継する。

中継は最大 50MB を往復するため、`vercel.json` で `api/google/[action].ts` の
`maxDuration` を 300 秒に広げている。

**③で失敗したときは、①で置いた実体を「そのまま保存」として登録し直す。**
変換できなかったからといって、利用者がアップロードしたファイルを失わせない。
その場合は「Google形式に変換できなかったため、そのまま保存しました」と理由付きで知らせる。

### 5.1.3 既存のGoogleファイルの取り込み

もともと Drive にあるファイルを、ファイルボックスへ追加する。
「Googleアプリ ▾」メニューの「既存のファイルを追加」から、2つの経路で入れられる。

**種別では絞らない。** Google形式（スプレッドシート・ドキュメント・スライド）でも、
Office文書・PDF・画像・zip でも、Drive にあるものは全て追加でき、どれも
**storage に実体を持たず、クリックすると Googleドライブが開く**形に揃える。

当初は Google形式と draw.io 図だけを受け付け、それ以外は「追加できません」と弾いていたが、
Drive で扱えるのに DevTicket のファイルボックスには置けないファイルが出てしまうため、
制限を外した。実体を DevTicket 側に取り込む案（storage へダウンロードする）も検討したが、
storage のバケット上限（50MB）を超えるファイルが**また追加できないもの**として残るため採らない。
Drive リンクとして持てば、サイズによらず必ず追加できる。

```
Googleドライブから選ぶ … Picker（マイドライブ／共有ドライブの2タブ・複数選択可）
URLを貼って追加       … URL からファイルIDを取り出し、そのファイルだけを出した Picker で1回 Select してもらう
                       ↓
                   import-files（サーバー）
```

**URL を貼るだけでは取り込めない。** `drive.file` で触れるのは「アプリが作ったもの」と
「Picker で選ばれたもの」だけで、URL の ID が分かっていても `files.get` は 404 になる。
URL だけで読めるようにするには `drive` / `drive.readonly` が必要になり、Google の審査が発生する。
...
- 持ち主が元の人のまま（その人のアカウントが消えると開けなくなる）
- 取り込んだ人に共有権限が無いと、他のメンバーに配れない
- `sync-names` は保存先フォルダの中しか見ないので「Driveで削除済み」と誤表示される
- DevTicket で改名すると、他人のファイルの名前を書き換えてしまう

コピーは「アプリが作ったファイル」になるので、新規作成したファイルと全く同じに扱える。
共有ドライブ運用なら持ち主は組織になる。保存先フォルダの中に既にあるものだけは、そのまま追加する
（同じファイルの二重登録は弾く）。

コピーの代わりに生じること:

- **元のファイルは残る。** 元を編集してもコピーには反映されない。取り込み時に
  「今後は DevTicket 側のファイルを編集してください」と知らせる
- **変更履歴とコメントはコピーに引き継がれない**
- **持ち主が「閲覧者のコピーを禁止」にしているファイルは取り込めない**（`capabilities.canCopy`）

取り込めなかったもの（閲覧権限なし・ゴミ箱・コピー禁止・二重登録・フォルダ）は、
理由をつけてまとめて知らせる。1件失敗しても他は続ける。

**ショートカットは指している先に読み替える。** そのままコピーすると中身を持たない
ショートカットの複製ができてしまうため、`shortcutDetails.targetId` を引き直してから扱う。

**Google形式以外のコピーは Drive の容量を消費する**（Google形式は消費しない）。
保存先フォルダの中に既にあるファイルはコピーしないので、大きなファイルを容量を増やさず
追加したい場合は、先に Drive 側で保存先フォルダへ移動してもらう。

追加後の一覧での扱い:

| | Google形式 | それ以外（Office・PDF・画像・draw.io 図…） |
|---|---|---|
| 名前 | 拡張子なし | Drive 上の名前のまま（`.xlsx` などの拡張子ごと。改名でも保つ） |
| サイズ | 持たない（種別だけ表示） | Drive の `size` を保存し、種別に続けて表示 |
| 種別の判定 | `file_type`（MIME） | 拡張子（storage 上のファイルと同じ規則） |
| ダウンロード | Office形式へ書き出し | `drive.google.com/uc?export=download` でそのまま |
| 開く | Google の編集画面 | Googleドライブのプレビュー画面（Office文書はそこから各アプリで編集できると案内する） |
| DevTicket 内のプレビュー／画面内編集／ファイルコメント | 使えない | 使えない（実体が storage に無いため） |

Picker のトークン（`picker-token`）は、当初は保存先フォルダの選択（管理者の設定作業）専用として
管理者に限っていたが、取り込みで一般メンバーも使うため、ログインしていれば発行するよう変更した。
渡るのは本人の `drive.file` トークン（本人が Picker で選んだものしか触れない）で、約1時間で失効する。

### 5.1.4 既にある Office文書を Google で開く

ファイルボックスにある Excel / Word / PowerPoint の行に「G」のボタンを出し、押すと
Google形式（スプレッドシート / ドキュメント / スライド）の**コピー**を作って別タブで開く（`convert-existing`）。

- 元の Office文書は Storage にそのまま残す（5.1.2 の変換アップロードとの違い）
- コピーは保存先フォルダに作り、ファイルボックスでも**元のファイルと同じフォルダ**に1行追加する
- 名前は拡張子を落としたもの。同名があれば「(1)」を付ける
- ボタンを出すのは、組織が連携を有効にしていて、変換できる拡張子のときだけ（.xlsm は対象外）

**押すたびに、その時点の最新版から新しくコピーを作る。** 以前に作ったコピーを開き直す形にすると、
その後に元の Excel が更新されていても古い中身のまま開いてしまう。代わりに、何度も押すと
コピーが増える。

**元のファイルとコピーは同期しない。** 片方を編集してももう片方には反映されない
（2.1 で「変換コピー」を避けた理由と同じ問題が、利用者が意図して選んだ場合に限って起きる）。
押したときに「元の〇〇とは別のファイルです」と必ず知らせる。
...
- 行クリックで別タブを開く。プレビューモーダルは開かない
- リンク共有がオンのファイルには、`v2` バッジと同じ位置に **「リンク公開中」バッジ**を常時表示する
- ダウンロードボタンは Office 形式への書き出しにする。
  `docs.google.com/.../export` を別タブで開き、**閲覧者自身のGoogleログインで直接落とさせる**。
  Drive API の `files.export` をサーバーで中継すると、ファイル本体が
  サーバーレス関数のレスポンス上限に引っかかるため

### 5.2.1 待機中のタブ

ファイルの作成は数秒かかる。`window.open` はクリック直後の同期実行中しか許可されないため、
**先に空タブを確保してから URL を流し込む**（`src/app/lib/pendingTab.ts`）。

確保したタブには DevTicket の体裁のローディング画面を書き込む。
about:blank に書くためアプリのCSSもフォントも読み込まれないので、
色・フォント・アニメーションはすべて `pendingTab.ts` 内で完結させている。

`window.open` に `noopener` を渡すとハンドルが `null` で返り、後から `location` を流し込めない。
...
| `resolve-folder` | POST | Picker で選ばれたフォルダの素性（フォルダか・どの共有ドライブか）を確かめる |
| `convert-staged` | POST | Storage に置いたOffice文書を Drive へ中継し、Google形式に変換して登録する（5.1.2） |
...
「Driveは倉庫、DevTicketが索引」という関係は変わらない。索引の見出し（名前）と、
実体がまだ在るかどうかだけを Drive 側から拾う。

### 9.1.1 取り込みの仕組み

Drive の変更通知（`changes.watch`）は使わない。チャンネルが最長7日で失効して自前で張り直す必要があり、
監視単位が「そのユーザーのドライブ全体」で DevTicket と無関係な変更でも起動してしまうため、
実装量と呼び出し回数に見合わない。

かわりに **`sync-names` を、ファイルボックスの一覧を取り直すタイミングで呼ぶ**。

- 画面遷移・リロード時 … **必ず**実行する
- タブ復帰時 … 前回から10秒以上空いていれば実行する
  （タブを往復するだけの操作で毎回サーバーレス関数を起こさないため。
  Google 側で名前を変える操作はどう急いでもこれより時間がかかるので取りこぼさない）
- プロジェクトに Googleファイルが1件も無ければ呼ばない

プロジェクトの Googleファイルは Drive 上の同じフォルダに集まるので、
**`files.list` 1回で全件の「今の名前」と「まだ在るか」がまとめて分かる**（消費は100ユニット）。
一覧の描画は待たせず、変化があったときだけ行を引き直す（`loading` はスピナーへ戻さない）。

Google を連携していないユーザーが見ているときは何もしない。
連携済みの誰かが開いたときに揃うので実害はない。

### 9.1.2 名前の重複をどう扱うか

`file_name` は DevTicket 内部の引き当てキーでもある。Drive 側の名前をそのまま流し込むと、
既存の別ファイルと名前がぶつかりうる。取り込み時は `nextFreeName()` を通し、
ぶつかる場合は `(1)` を付けて**DevTicket 側の一意性を優先する**（Google と表示名がズレるが、安全側に倒す）。

あわせて、**Googleファイルの改名・削除は `file_name` ではなく行の `id` で引く**ように変更した
（`api/project-files/[action].ts`）。通常のファイルは「同名＝同じファイルの別バージョン」なので
まとめて処理してよいが、Googleファイルに版の概念は無く、名前は Google 側で自由に変えられる。
