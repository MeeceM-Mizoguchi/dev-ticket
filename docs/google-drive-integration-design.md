...
| ⑤ | 共有ドライブの指定 | **Google Picker で選択**。`drive.file` では `drives.list` / `drives.create` が使えず、一覧提示も新規作成もできないため、顧客側で作成済みのものを選んでもらう |
| ⑥ | フォルダ構成 | 両モードとも `<ルート>/DevTicket/<プロジェクト名>/` を**自動生成**する |
...
| アプリが作っていない任意のフォルダを親に指定 | 既存フォルダへの保存ができない可能性 | Picker で選択させれば触れるようになる（**要検証**、9章） |
...
- `drive.file` スコープで、**Pickerで選んだ共有ドライブの直下にフォルダを作れるか**。
  Picker選択物はアプリからアクセスできるようになる仕様だが、共有ドライブのルートを親に
  指定する挙動は実機で確認すること。ダメなら「顧客側で `DevTicket` フォルダまで作ってもらい、
  そのフォルダをPickerで選ばせる」に落とす。
