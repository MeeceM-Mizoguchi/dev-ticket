...
| 手順 | 内容 |
|---|---|
| ① コンフリクトチェック | 対象の全件を `POST /api/github/merge-precheck` で確認。1件でも通らなければ**ここで止める** |
| ② マージ | 全件通った場合だけ実行。1件ずつ順番に、実行直前にもう一度状態を引き直す |
| ③ 一覧の更新 | 実行後に取り直す |
...
- `unknown`（GitHub 側で計算中）も**通さない**。分からないまま実行すると、
  途中でコンフリクトに当たって一部だけ入った状態になる。1回だけ待って引き直し、
  それでも決まらなければ「判定中」として止める。
