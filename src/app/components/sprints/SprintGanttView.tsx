...
import { daysBetween, formatDate, getSprintStatusMeta, sprintProgress, TICKET_STATUSES } from "@/app/lib/helpers";
...
export function SprintGanttView({ sprints, onSelectSprint, onSelectTicket }: {
  ...
  const allDates = sprints.flatMap(s => [s.startDate, s.endDate, ...s.tickets.flatMap(t => [t.startDate, t.dueDate].filter(Boolean))]);
  const validDates = allDates.filter(Boolean);
  const minDate = validDates.reduce((m, d) => d < m ? d : m, validDates[0]);
  const maxDate = validDates.reduce((m, d) => d > m ? d : m, validDates[0]);
  ...
  const calDays: { date: string; dayOfMonth: number; isFirst: boolean }[] = [];
  const cur = new Date(minDate);
  for (let i = 0; i < totalDays; i++) {
    const ds = cur.toISOString().split("T")[0];
    calDays.push({ date: ds, dayOfMonth: cur.getDate(), isFirst: cur.getDate() === 1 });
  ...
  let curM = new Date(startD.getFullYear(), startD.getMonth(), 1);
  while (curM <= endD) {
    const mStart = new Date(curM);
    const mEnd = new Date(curM.getFullYear(), curM.getMonth() + 1, 0);
    ...
    months.push({ year: curM.getFullYear(), month: curM.getMonth() + 1, label: `${curM.getMonth() + 1}月`, left: getLeft(effStart.toISOString().split("T")[0]), width: days * DAY_W });
  ...
  const LEFT_W = 230, ROW_H = 44, TICK_ROW_H = 30, YEAR_H = 22, MON_H = 24, DAY_H = 20, HDR_H = YEAR_H + MON_H + DAY_H;

  const GridLines = () => (
    <>
      {calDays.map((d, i) => (
        <div key={i} style={{ position: "absolute", top: 0, bottom: 0, left: i * DAY_W, width: 1,
          background: d.isFirst ? "rgba(26,23,20,0.15)" : d.dayOfMonth % 5 === 0 ? "rgba(26,23,20,0.07)" : "rgba(26,23,20,0.03)",
          pointerEvents: "none" }} />
      ))}
      {todayLeft >= 0 && <div style={{ position: "absolute", top: 0, bottom: 0, left: todayLeft, width: 2, background: "#059669", opacity: 0.6, pointerEvents: "none" }} />}
    </>
  );
        ...
        <div style={{ flex: 1, overflowX: "auto" }}>
          <div style={{ width: Math.max(totalDays * DAY_W, 600), position: "relative" }}>
            ...
            <div style={{ height: MON_H, background: "#F4F5F6", borderBottom: "1px solid rgba(26,23,20,0.07)", position: "relative" }}>
              {months.map((m, i) => (
                <div key={i} style={{ position: "absolute", left: m.left, width: m.width, height: "100%", display: "flex", alignItems: "center", padding: "0 6px", borderRight: "1px solid rgba(26,23,20,0.10)", boxSizing: "border-box" as const }}>
            ...
            <div style={{ height: DAY_H, background: "#FAFAF8", borderBottom: "1px solid rgba(26,23,20,0.07)", position: "relative" }}>
              {calDays.map((d, i) => (
                <div key={i} style={{ position: "absolute", left: i * DAY_W, width: DAY_W, height: "100%", display: "flex", alignItems: "center", justifyContent: "center",
                  ...
                  background: d.date === todayStr ? "rgba(5,150,105,0.08)" : "transparent" }}>
                  {(d.dayOfMonth === 1 || d.dayOfMonth % 5 === 0) && (
            ...
            {sprints.map(sprint => {
              ...
              return (
                <div key={sprint.id}>
                  ...
                  {isExp && sprint.tickets.map(t => {
                    const tsm = TICKET_STATUSES.find(s => s.value === t.status) ?? TICKET_STATUSES[0];
                    const hasStart = !!t.startDate, hasDue = !!t.dueDate;
                    const tL = hasStart ? getLeft(t.startDate) : 0;
                    const tW = hasStart && hasDue ? getWidth(t.startDate, t.dueDate) : 0;
                    return (
                      <div key={t.id} style={{ height: TICK_ROW_H, borderBottom: "1px solid rgba(26,23,20,0.03)", position: "relative", background: "rgba(26,23,20,0.012)" }}>
                        ...
                        {hasStart && hasDue && (
                          <div style={{ position: "absolute", left: tL, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: 4, zIndex: 1 }}>
                            <div style={{ width: Math.max(tW, 2), height: 12, borderRadius: 3, background: tsm.color + "25", border: `1px solid ${tsm.color}50`, overflow: "hidden", flexShrink: 0, position: "relative" }}>
